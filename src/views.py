...
def home_page(file_csv: pd.DataFrame) -> str:
    ...
    logger.debug("Передаем данные в функцию 'user_greeting()'")
    greeting = user_greeting()
    logger.debug("Передаем данные в функцию 'cashback_and_cart_numb()'")
    cards = cashback_and_cart_numb(file_csv)
    logger.debug("Передаем данные в функцию 'top_five_sum_transacts()'")
    top_transacts = top_five_sum_transacts(file_csv)
    logger.debug("Передаем данные в функцию 'currency_conversion()'")
    currency_rates = currency_conversion()
    logger.debug("Передаем данные в функцию 'data_sp500()'")
